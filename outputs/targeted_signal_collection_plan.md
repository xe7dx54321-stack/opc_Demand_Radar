# Targeted Signal Collection Plan

## Summary

- Plans: 3
- Total target new signals: 26
- Generated at: 2026-06-15 14:45 UTC

## Collection Plans

### 1. 内容团队在选题生产中面临的信息分散与人工整理低效问题

Target New Signals: 6
Target Source Types: product_review, forum_post, case_study, pricing_page, landing_page
Target Languages: zh, en

**Search Keywords ZH:**
- 内容团队 选题 效率 痛点
- 热点筛选 人工整理 素材 工作流
- 自媒体 选题 工具 付费
- 内容运营 信息源 分散
- AI 内容选题 工作流
- 内容团队在选题生 付费 工具 抢占

**Search Keywords EN:**
- content team topic research workflow pain
- editorial planning information overload
- content research tool pricing
- newsletter topic discovery workflow
- AI content planning workflow automation

**Positive Signal Criteria:**
- 描述具体的人工步骤或替代工具
- 提到使用付费工具或人力外包解决问题
- 提到当前使用的工具或流程

**Negative Signal Criteria:**
- 只表达不局服但未描述任何尝试方式
- 只说「这个问题很烦」但尚未尝试任何解决方法
- 无任何当前解决方案描述

**Collection Notes:** 优先寻找以下类型信号：manual_workaround、paid_alternative、current_solution。建议来源：product_review, forum_post, case_study。目标数量：6 条。

**Expected Impact:** 补充后预期分数可提升至 75 分，有望达到 strong 级别。

---

### 2. 运营在企业知识工作流中遇到的「文档分散、检索困难、人工整理低效」问题

Target New Signals: 10
Target Source Types: product_review, forum_post, case_study, pricing_page, landing_page
Target Languages: zh, en

**Search Keywords ZH:**
- SaaS 工具 信息整理 痛点
- 企业知识库 检索 困难
- 运营 文档分散 工作流
- 运营在企业知识工 付费 工具 抢占
- 运营在企业知识工 业务损失 效率 影响

**Search Keywords EN:**
- enterprise knowledge base search difficulty
- SaaS tool information fragmentation pain
- operations workflow documentation scattered
- tool pricing subscription workflow
- business impact workflow inefficiency cost

**Positive Signal Criteria:**
- 描述具体的人工步骤或替代工具
- 提到使用付费工具或人力外包解决问题
- 提到当前使用的工具或流程
- 明确语境包括预算、付费工具或采购记录

**Negative Signal Criteria:**
- 只表达不局服但未描述任何尝试方式
- 只说「这个问题很烦」但尚未尝试任何解决方法
- 无任何当前解决方案描述

**Collection Notes:** 优先寻找以下类型信号：manual_workaround、paid_alternative、current_solution。建议来源：product_review, forum_post, case_study。目标数量：10 条。

**Expected Impact:** 补充后预期分数可提升至 74 分，有望达到 strong 级别。

---

### 3. 投资人在AI产业跟踪中面临的多渠道信息人工收集整理低效问题

Target New Signals: 10
Target Source Types: product_review, forum_post, case_study, pricing_page, landing_page
Target Languages: zh, en

**Search Keywords ZH:**
- AI 项目初筛 信息分散 尽调
- 投资人 AI 产业跟踪 数据源
- VC 项目筛选 人工整理 痛点
- AI 公司动态 追踪 工具 付费
- 投资人在AI产业 付费 工具 抢占
- 投资人在AI产业 业务损失 效率 影响

**Search Keywords EN:**
- AI startup deal sourcing research workflow
- VC AI market tracking pain point
- investor due diligence workflow tools
- AI company tracking tool pricing
- tool pricing subscription workflow

**Positive Signal Criteria:**
- 描述具体的人工步骤或替代工具
- 提到使用付费工具或人力外包解决问题
- 提到当前使用的工具或流程
- 明确语境包括预算、付费工具或采购记录

**Negative Signal Criteria:**
- 只表达不局服但未描述任何尝试方式
- 只说「这个问题很烦」但尚未尝试任何解决方法
- 无任何当前解决方案描述

**Collection Notes:** 优先寻找以下类型信号：manual_workaround、paid_alternative、current_solution。建议来源：product_review, forum_post, case_study。目标数量：10 条。

**Expected Impact:** 补充后预期分数可提升至 74 分，有望达到 strong 级别。

---
